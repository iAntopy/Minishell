...
{
	if (argc < 2)
		return (report_bad_inputs(argc));
	*here_doc = (ft_strcmp("here_doc", argv[1]) == 0);
	argc -= *here_doc;
	if ((argc < 5) || ((argc - 3) > CMD_MAX))
		return (report_bad_inputs(argc + *here_doc));
	return (0);
}
...
int	validate_io_files(int *io, int argc, char **argv, int here_doc)
{
	...
	{
		if (pipe(pp) < 0)
			return (report_error("pipe failed"));
		io[0] = pp[0];
		close_pipe(NULL, pp + 1);
		report_file_error(argv[1]);
	}
	else
	...
	if (access(argv[argc - 1], F_OK) == 0 && access(argv[argc - 1], W_OK) < 0)
		return (report_file_error(argv[argc - 1]));
