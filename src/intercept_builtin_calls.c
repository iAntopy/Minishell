...
/*   Updated: 2023/01/11 23:45:43 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strncmp(cmd, "echo", 4) == 0 && (!cmd[4] || ft_isspace(cmd[4])))
		return (msh_builtin_echo(msh, cmd));
	else if (ft_strncmp(cmd, "cd", 2) == 0 && (!cmd[2] || ft_isspace(cmd[2])))
		return (msh_builtin_cd(msh, cmd));
	else if (ft_strncmp(cmd, "export", 6) == 0 && (!cmd[6] || ft_isspace(cmd[6])))
		return (msh_builtin_export(msh, cmd));
	else if (ft_strncmp(cmd, "unset", 5) == 0 && (!cmd[5] || ft_isspace(cmd[5])))
		return (msh_builtin_unset(msh, cmd));
	else if (ft_strncmp(cmd, "pwd", 3) == 0 && (!cmd[3] || cmd[3] == ' '))
		return (msh_builtin_pwd(msh));
	else if (ft_strncmp(cmd, "env", 3) == 0 && (!cmd[3] || ft_isspace(cmd[3])))
		return (msh_builtin_env(msh));
	else if (ft_strncmp(cmd, "exit", 4) == 0 && (!cmd[4] || ft_isspace(cmd[4])))
